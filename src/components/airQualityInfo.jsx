  ...
  useEffect(() => {
    ...
    navigator.geolocation.getCurrentPosition(
      ...
      (err) => {
        ...
        fetchAirQualityData(37.5665, 126.9780);
      }
    );
  }, []);
      ...
      <div className={styles.header}>
        <div className={styles.titleWrapper}>
          <h3 className={styles.title}>
            <span className={styles.highlight}>{airQualityData.district}</span>  대기질 정보
            <div 
              className={styles.statusDot} 
              style={{ backgroundColor: getStatusDotColor(airQualityData.grade) }}
            ></div>
          </h3>
        </div>
